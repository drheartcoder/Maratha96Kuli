...
$end=30;
 ...
 $sqllimit="select * from bride where status='1' and marital_status = 'Widow'";
	$limittest=mysqli_query($con,$sqllimit);
    $rowlimit=mysqli_num_rows($limittest);   

    $sql="select * from bride where status='1' and marital_status = 'Widow' order by bride_id desc limit " .$start.",30";
...
<?php
		...
		$pagination_new.=$prev_pagination.'<a href="widow.php?first='.$count.'&last=30&slot='.$new_slot.'" class="bluelink">'.$k.'</a>&nbsp;&nbsp;';
		$prev_pagination="";
				...
				if($k%30==0)
				{
					...
					$next_pagination='<a href="widow.php?first='.$count.'&last=30&slot='.$new_slot.'" class="bluelink">&gt;&gt;</a>&nbsp;&nbsp;';
					...
					$prev_pagination='<a href="widow.php?first='.$ft .'&last=30&slot='. $lt .'" class="bluelink">&lt;&lt;</a>&nbsp;&nbsp;';
				}
				...
				$x =$total_pages-isset($no_of_pages);
			...
			for($m=0;$m<$x;$m++)
			 {
				...
				$pagination_new.='<a href="widow.php?first='.$count.'&last=30&slot='.$new_slot.'" class="bluelink">'.$n .'</a>&nbsp;&nbsp;';
				...
				$a_pages[]=isset($prev_pagination).isset($pagination_new); 	  
