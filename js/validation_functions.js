...
if(!isInteger(obj.value.substring(0,3)))
  {
   err_div.innerHTML = msg;
   obj.focus();
   return false;
  } 
if(!isInteger(obj.value.substring(4,7)))
  {
   err_div.innerHTML = msg;
   obj.focus();
   return false;
  } 
if(!isInteger(obj.value.substring(8,obj.value.length)))
  {
   err_div.innerHTML = msg;
   obj.focus();
   return false;
  }  
 // var val =obj.value.substring(0,3);
 if(Number(obj.value.substring(0,3))==0)
//if(val.charAt(0)=="0" && val.charAt(1) == "0" && val.charAt(2) =="0")
{
  err_div.innerHTML = msg;
  obj.focus();
  return false; 
}
//val = obj.value.substring(4,7);
if(Number(obj.value.substring(4,7))==0)
//if(val.charAt(0)=="0" && val.charAt(1) == "0" && val.charAt(2) =="0")
{
  err_div.innerHTML = msg;
   obj.focus();
...
if(Number(obj.value.substring(8,obj.value.length))==0)
//val = obj.value.substring(8,obj.value.length);
//if(val.charAt(0)=="0" && val.charAt(1) == "0" && val.charAt(2) =="0" && val.charAt(3) == "0")
{
  err_div.innerHTML = msg;
   obj.focus();
   return false;
...
{ 
        var RegExp = /^((([aA-zZ]|[0-9]|!|#|$|%|&|'|\*|\+|\-|\/|=|\?|\^|_|`|\{|\||\}|~)+(\.([aA-zZ]|[0-9]|!|#|$|%|&|'|\*|\+|\-|\/|=|\?|\^|_|`|\{|\||\}|~)+)*)@((((([aA-zZ]|[0-9])([aA-zZ]|[0-9]|\-){0,61}([aA-zZ]|[0-9])\.))*([aA-zZ]|[0-9])([aA-zZ]|[0-9]|\-){0,61}([aA-zZ]|[0-9])\.)[\wW]{2,4}|(((([0-9]){1,3}\.){3}([0-9]){1,3}))|(\[((([0-9]){1,3}\.){3}([0-9]){1,3})\])))$/ 
